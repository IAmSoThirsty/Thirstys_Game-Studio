...
class PRCreatorWorker(Worker):
    """Worker for PR creation tasks.

    Creates GitHub pull request templates from proposals.
    """

    def __init__(self):
        """Initialize the PR creator worker."""
        super().__init__(AgentRole.PR_CREATOR)
        self.generator = PRTemplateGenerator()

    def can_handle(self, task: Task) -> bool:
        """Check if this worker can handle the task."""
        return task.role == AgentRole.PR_CREATOR

    def execute(self, task: Task) -> TaskResult:
        """Execute the PR creation task."""
        start_time = time.time()
        task.status = TaskStatus.IN_PROGRESS
        ...
        try:
            ...
            # Generate PR drafts from proposals
            pr_drafts = self.generator.batch_generate(proposals)

            # Also generate an agent run PR if we have run_summary
            run_summary = task.input_data.get("summary", {})
            if run_summary:
                agent_run_pr = self.generator.generate_agent_run_pr(
                    run_summary, proposals
                )
                pr_drafts.append(agent_run_pr)
            ...
            return TaskResult(
                ...
                success=True,
                output_data={
                    "pr_drafts": [pr.to_dict() for pr in pr_drafts],
                    "count": len(pr_drafts),
                },
                execution_time=execution_time,
            )
...
def create_worker_for_role(role: AgentRole) -> Worker:
    ...
    workers = {
        ...
        AgentRole.PR_CREATOR: PRCreatorWorker,
    }
